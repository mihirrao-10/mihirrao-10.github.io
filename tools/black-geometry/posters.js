...
export function sculpturePoster(model,{shortcut="open"}={}) {
  ...
  const faces=model.faces.map(face=>{
    ...
    const shade=.54+.58*key+.3*rim;
    const specular=Math.max(0,normal.dot(new THREE.Vector3(-.25,.32,1).normalize()))**44;
    const points=[0,3,6].map(i=>project(p.slice(i,i+3)));
    const fill=toHex(face.c.map(c=>Math.min(1,c*shade+.52*specular+.12*rim)));
  ...
  let body=`<g id="surface" opacity=".78">${faces.sort((a,b)=>a.depth-b.depth).map(f=>f.markup).join("")}</g>`;
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 800 600" fill="none" aria-hidden="true"><defs><filter id="glow" x="-20%" y="-20%" width="140%" height="140%"><feGaussianBlur stdDeviation="3"/></filter></defs><use href="#surface" filter="url(#glow)" opacity=".18"/>${body}</svg>\n`;
}
