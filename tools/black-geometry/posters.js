...
export function sculpturePoster(model,{shortcut="open"}={}) {
  ...
  const scale=Math.min(720/(upper[0]-lower[0]),528/(upper[1]-lower[1]));
  ...
  const faces=model.faces.map(face=>{
    ...
    const shade=.32+.92*key+.10*fillLight;
    const specular=Math.max(0,normal.dot(new THREE.Vector3(-.25,.32,1).normalize()))**90;
    const points=[0,3,6].map(i=>project(p.slice(i,i+3)));
    const fill=toHex(face.c.map(c=>Math.min(1,c*shade+.55*specular+(c*.92+.08)*rim*.33)));
    const edge=toHex(face.c.map(c=>Math.min(1,(c*1.2+.055)*(.7+.3*key))));
    const presence=Math.max(...face.c)>.01?.13:.015;
    return {depth:(p[2]+p[5]+p[8])/3, markup:`<path d="M${points.map(p=>`${n(p[0])} ${n(p[1])}`).join("L")}Z" fill="#${fill}" stroke="#${edge}" stroke-opacity="${presence}" stroke-width=".28" stroke-linejoin="round"/>`};
  });
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 800 600" fill="none" aria-hidden="true"><defs><filter id="glow" x="-20%" y="-20%" width="140%" height="140%"><feGaussianBlur stdDeviation="3"/></filter></defs><use href="#surface" filter="url(#glow)" opacity=".32"/>${body}</svg>\n`;
}
